...
def _crop_center(array, center, crop_window=None):
    ...
    if crop_window is not None:
        ...
        # Get the limits
        y1, y2, x1, x2 = (
            center[0] - crop_window[0],
            center[0] + crop_window[0] + 1,
            center[1] - crop_window[1],
            center[1] + crop_window[1] + 1,
        )
...
def _gaussian_fit(x, y, n_points=1000, dark_spots=False):
    ...
    parameters, covariances = curve_fit(
        _gaussian, x, y, p0=[AInit, x0Init, wInit, y0Init]
    )
...
def _select_fit_function(
    ...
    if fit_function == "gaussian":
        intensity, distance, params, errs = _gaussian_fit(
            raw_distance, raw_intensity, n_points=n_points, dark_spots=dark_spots
        )

    elif fit_function == "sinc":
        intensity, distance, params, errs = _sinc_fit(
            raw_distance, raw_intensity, n_points=n_points, dark_spots=dark_spots
        )
...
def _compute_raw_profile(
    array, type="radial", line_angle=0, center=None, space_scale=None
):
...
def _fit_profile(
    ...
    # Calculate the center for the profile
    raw_output = _compute_raw_profile(
        array, type=fit_type, line_angle=line_angle, center=center
    )
    raw_intensity, raw_distance = raw_output["intensity"], raw_output["distance"]

    # Do the required fit on the profile
    intensity, distance, params, errs = _select_fit_function(
        raw_distance,
        raw_intensity,
...
def _get_profile(
    ...
    if profile_type.lower() in ["gaussian", "sinc"]:
        output = _fit_profile(
            cropped_array,
            fit_function=profile_type.lower(),
            fit_type=fit_type,
...
def _process_noise(track_profile, dark_spots=False, fit_type="gaussian", n_points=1000):
    ...
    if "parameters" in track_profile.keys():
        ...
        for i, int_value in enumerate(intensities):
            dist, int, params, err = _select_fit_function(
                distances[i],
                int_value,
...
def _process_signal(track_profile, use_fit=True, dark_spots=False, fit_type="gaussian"):
    ...
    if "parameters" in track_profile.keys():
        ...
        for i, int_value in enumerate(intensities):
            dist, int, params, err = _select_fit_function(
                distances[i], int_value, fit_function=fit_type, dark_spots=dark_spots
            )
            parameters.append(params)
        parameters = np.array(parameters)
    ...
    # Calculate the contrast using the max/min amplitude
        ...
        contrast = _contrast_calculation_extremum(
            intensities, parameters, dark_spots=dark_spots
        )
...
def _get_signal_properties(
    track_profile, use_fit=True, dark_spots=False, fit_type="gaussian", percentage=True
):
    ...
    contrast = _process_signal(
        track_profile, use_fit=use_fit, dark_spots=dark_spots, fit_type=fit_type
    )
    ...
    # Convert into percentage if required
    if percentage:
...
def _integration_on_fit(parameters, fit_type, limits=[-1000, 1000]):
    ...
    if fit_type == "gaussian":
        integration, _ = quad(
            ...
            -np.inf,
            np.inf,
            args=(parameters[0], parameters[1], parameters[2], 0),
        )
...
def _integrate_fitted_profile(
    distances, parameters, dark_spots=False, fit_type="gaussian"
):

    # Integrate each profile
    integrated_profiles = []
    for i, parameter_set in enumerate(parameters):
        integrated_profiles.append(
            _integration_on_fit(
                parameter_set, fit_type, limits=[distances[i][0], distances[i][-1]]
            )
        )
...
def _get_integrated_intensity(
    track_profile, use_fit=True, dark_spots=False, fit_type="gaussian"
):
    ...
    if use_fit:
        integration = _integrate_fitted_profile(
            distances, parameters, dark_spots=dark_spots, fit_type=fit_type
        )
...
def intensityProfile(
    ...
    for id in track_ids:
        ...
        crt_data = positions[positions["particle"] == id][
            ["frame", "y", "x"]
        ].to_numpy()
...
def signalProperties(
    ...
    for track_id in profiles.keys():
        contrast, noise, snr = _get_signal_properties(
            profiles[track_id],
            use_fit=use_fit,
            dark_spots=dark_spots,
            fit_type=fit_type,
...
def integratedIntensity(profiles, use_fit=True, dark_spots=False, fit_type="gaussian"):
    ...
    for track_id in profiles.keys():
        integrated_intensity[track_id] = _get_integrated_intensity(
            profiles[track_id],
            use_fit=use_fit,
            dark_spots=dark_spots,
            fit_type=fit_type,
